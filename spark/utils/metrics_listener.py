...
def push_metrics(batch_id: int, n_in: int, n_merged: int, n_dlq: int, batch_seconds: float, last_commit_ts: float, processed_offsets: Optional[Dict[str, int]] = None) -> None:
    lines = [
...
def extract_processed_offsets(last_progress) -> Dict[str, int]:
    ...
    try:
        ...
        if isinstance(topic_map, dict) and "payments.public.transactions" not in raw and len(raw) == 1:
            topic_map = next(iter(raw.values()))
        if not isinstance(topic_map, dict): return {}
        out: Dict[str, int] = {}
        for part, off in topic_map.items():
            try: out[str(part)] = int(off)
            except (TypeError, ValueError): continue
        return out
    except Exception as exc:
